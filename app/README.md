...
- **Goals** — app-wide duration display format, standard and custom goals that can be enabled/disabled and reordered with a persisted drag handle, and custom goal edit/swipe-to-delete. `Hours` uses `00:00:00` timers; `Days + hours` uses compact non-zero units such as `55m 5s` or `1d 2h 3m 4s`. A floating plus opens the add-goal editor. All saved goals can be disabled; This time and Open-ended remain available on the Fast screen.
